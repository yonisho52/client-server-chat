...
public class socetHandeler_send extends Thread {
    ...
    socetHandeler_send(Socket incoming , ArrayList<arrays>  arr_array , int team)
    {
    ...
    {				// then we will send it to the client 
    	try{
    	 for(int i=place ; i<arr_array.size(); i++,place++)
      	   if(arr_array.get(i).getIncoming() != incoming )// we don't want that the client will get his
      	   {									// own words
